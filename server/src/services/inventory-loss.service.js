...
async function withLossTransaction(operation) {
  const maxAttempts = 3;
  ...
  throw new HttpError(500, "No se pudo registrar la perdida.");
}
...
async function resolveExplicitLot(tx, productId, inventoryLotId, quantity) {
  ...
  if (available.lte(0) || available.lt(quantity)) {
    throw new HttpError(422, "Stock insuficiente en el lote seleccionado.");
  }
...
async function resolveFefoLots(tx, productId, quantity) {
  ...
  if (availableQty.lt(quantity)) {
    throw new HttpError(422, "Stock insuficiente por lotes para registrar la perdida.", {
      productId,
  ...
  if (remaining.gt(0)) {
    throw new HttpError(422, "Stock insuficiente por lotes para completar la perdida.");
  }

  return allocations;
}

export async function registerInventoryLoss(data, userId) {
  ...
  if (String(data.reference || "").startsWith("PRODUCCION:")) {
    throw new HttpError(422, "La merma de produccion no debe reprocesarse como perdida independiente.");
  }

  return withLossTransaction(async (tx) => {
    ...
    if (beforeProductStock.lt(quantity)) {
      throw new HttpError(422, "Stock insuficiente para registrar la perdida.");
    }
